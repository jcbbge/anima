...
import { checkAndPromote } from "./tierService.ts";
import {
  adjustResonance,
  detectPotentialCatalyst,
} from "./resonanceService.ts";
import {
  findSemanticDuplicate,
  mergeIntoSemanticCentroid,
} from "./semanticConsolidationService.ts";
import { generateHandshake } from "./handshakeService.ts";
...
export async function addMemory(memoryData) {
  ...
  const accessedConversationsParam = sql.array([], "TEXT");
  ...
  const insertResult = await runQuery(
    ...
    [
      ...
      source || null,
      "active",
      accessedConversationsParam,
      initialPhi,
      isCatalyst,
    ],
  );

  const memory = insertResult[0];
...
export async function queryMemories(queryData) {
  ...
  if (result.length > 0) {
    const memoryIds = result.map((row) => row.id);
    ...
    `;
    const promotionCandidates = await runQuery(promotionCheckSql, [memoryIds]);

    // 3. BATCH PROMOTE: Eligible memories (single query)
    if (promotionCandidates.length > 0) {
      const candidateIds = promotionCandidates.map((r) => r.id);
      const promotionSql = `
        UPDATE memories
        SET tier = CASE
        ...
        RETURNING id, tier
      `;
      const promotionResult = await runQuery(promotionSql, [candidateIds]);

      promotions.push(
        ...promotionResult.map((row) => ({
          memoryId: row.id,
          toTier: row.tier,
        })),
      );
      ...
      result.forEach((memory) => {
        const promotion = promotionResult.find((p) => p.id === memory.id);
        if (promotion) {
    ...
    if (result.length > 1 && conversationId) {
...
export async function loadBootstrap(bootstrapData) {
  ...
  const result = await runQuery(sql, [conversationId]);

  // Group by tier
  const memories = {
    active: result.filter((m) => m.tier === "active"),
    thread: result.filter((m) => m.tier === "thread"),
    stable: result.filter((m) => m.tier === "stable"),
  };
...
async function checkSemanticDuplicateAsync(newMemoryId, embedding) {
  ...
  try {
    ...
    if (match && match.id !== newMemoryId) {
      ...
      const bothMemories = await runQuery(
        `SELECT id, created_at FROM memories WHERE id = ANY($1)`,
        [[newMemoryId, match.id]],
      );

      const newMem = bothMemories.find((m) => m.id === newMemoryId);
      const oldMem = bothMemories.find((m) => m.id === match.id);
      ...
      if (newMem && oldMem) {
        ...
        const newerContent = await runQuery(
        ...
        await mergeIntoSemanticCentroid(
          olderMemory,
          newerContent[0].content,
          false,
          match.similarity,
        );

        // Soft-delete the newer duplicate
        await runQuery(`UPDATE memories SET deleted_at = NOW() WHERE id = $1`, [
          newerMemory,
        ]);
