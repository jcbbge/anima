...
export async function generateHandshake(options = {}) {
  ...
  const threadsResult = await sql.unsafe(
    `SELECT content, id
     FROM memories
  ...
  if (conversationId) {
    ...
    const convReflectionResult = await sql.unsafe(
    ...
    reflection = convReflectionResult[0] || null;
  }

  // Fall back to most recent global reflection if no conversation-specific one found
  if (!reflection) {
    const globalReflectionResult = await sql.unsafe(
      `SELECT metrics, insights, reflection_type
       FROM meta_reflections
       ORDER BY created_at DESC
       LIMIT 1`,
    ...
    reflection = globalReflectionResult[0] || null;
  }
  ...
  const dreamsResult = await sql.unsafe(
    `SELECT 
      id,
      content,
      created_at,
  ...
  const recentDreams = dreamsResult.map((dream) => {
  ...
  // 5. Store the ghost
  const topMemoryIds = topMemories.map((m) => m.id);
  const topPhiValues = topMemories.map((m) => Number(m.resonance_phi) || 0);

  const topMemoryParam = topMemoryIds.length > 0
    ? sql.array(topMemoryIds, "UUID")
    : sql.array([], "UUID");
  const phiValuesParam = topPhiValues.length > 0
    ? sql.array(topPhiValues, "DOUBLE PRECISION")
    : sql.array([], "DOUBLE PRECISION");

  const ghostResult = await sql.unsafe(
 ...
 */
async function getTopMemoriesForConversation(conversationId: string, limit: number) {
  const result = await sql.unsafe(
    `SELECT 
     ...
     ORDER BY
       CASE
         WHEN COALESCE(last_accessed, created_at) >= (NOW() - INTERVAL '1 day') THEN 1
         ELSE 0
       END DESC,
       synthesis_weight DESC,
 ...
 */
async function getTopMemoriesGlobal(limit: number) {
  const result = await sql.unsafe(
    `SELECT 
       id, 
       content, 
