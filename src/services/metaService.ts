...
import { findHubs } from "./associationService.ts";

type QueryExecutor = (text: string, params?: any[]) => Promise<any[] | { rows: any[] }>;
...
export async function generateReflection({
  ...
  const executor: QueryExecutor = queryFn ?? ((text, params = []) => sql.unsafe(text, params));
  ...
  const result = await exec(
    ...
    [
      ...
      insights.length > 0 ? sql.array(insights, "TEXT") : sql.array([], "TEXT"),
      recommendations.length > 0 ? sql.array(recommendations, "TEXT") : sql.array([], "TEXT"),
 ...
 */
async function calculateFrictionMetrics(
...
async function getTierDistribution() {
  ...
  result.forEach((row) => {
    distribution[row.tier] = parseInt(row.count, 10);
  });
 ...
 */
export async function getRecentReflections(conversationId: string | null, limit = 1) {
 ...
 */
export async function getReflectionsByType(reflectionType: string, limit = 10) {
